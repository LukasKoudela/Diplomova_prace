...
Je jasné, že první uvedená metoda bude s nárůstem počtu vrstev čím dál více složitější. Postup další varianty řešení je opět rozebrán v \cite[str. 105]{emp}. Tato metoda, jak už název napovídá, využívá matic. Pokud dopadají na rozhraní vlny z obou stran podle obrázku !!! je možné odvodit následující maticový tvar
\begin{equation}
\left[ \begin{array}{c}
\faz E_{1}^{+} \\
\faz E_{1}^{-} \\
...
\end{equation}
kde $\faz R_{12}$ představuje činitel odrazu a $\faz T_ {12}$ činitel prostupu při dopadu vlny z prostředí 1 do 2. Pokud bude vlna procházet vrstvou o tloušťce $d$ popsanou konstantou šíření $\faz k$, podle situace na obrázku !!! lze opět získat maticový zápis
\begin{equation}
\left[ \begin{array}{c}
\faz E_{2}^{+} \\
\faz E_{1}^{-} \\
...
\end{equation}
V případě vícevrstvenného prostředí se využívá odvozených vztahů (\ref{rce:evlny_maticova_rozhrani}) a (\ref{rce:evlny_maticova_pruchod}). Postupně se za sebou násobí takové matice, podle toho na jaké rozhraní a jakým prostředím elektromagnetická vlna prochází. Příklad šíření je uveden na obrázku !!!!. Vlna šířící se z prostředí 1 do prostředí 4 projde přes dvě další oblasti o tloušťce $d_{1}$, $d_{2}$ a tři rozhraní. Pro výslednou sestavu matic bude platit
\newpage
\begin{displaymath}
\left[ \begin{array}{c}
\faz E_{1}^{+} \\
...
Pro situaci na obrázku !!!! bude prvním krokem transformace impedance $\faz Z_{3}$ a $\faz Z_{4}$ na rozhraní $r_{23}$ podle vztahu
\begin{equation}
